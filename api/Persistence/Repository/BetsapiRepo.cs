...
public class BetsapiRepo : BaseRepo {
	...
	public async Task<BetsapiData_MergeHistory?> FetchMatchMergeHistory(long match_id, long since_time, int page) {
		...
		return await this.httpClient.GetForTypeOrThrowAsync<BetsapiData_MergeHistory>(url);
	}
	...
	public async Task<BetsapiData_MatchHistory?> FetchMatchHistory(long match_id) {
		...
		return await this.httpClient.GetForTypeOrThrowAsync<BetsapiData_MatchHistory>(url);
	}

	/// <param name="size">One of: s (small), m (medium), b (big)</param>
	public async Task<byte[]?> FetchTeamImage(string image_id, string size = "s") {
		try {
			var response = await this.httpClient.GetRawAsync($"https://assets.b365api.com/images/team/{size}/{image_id}.png");
