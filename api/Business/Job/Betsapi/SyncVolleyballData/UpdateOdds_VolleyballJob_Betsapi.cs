...
public class UpdateOdds_VolleyballJob_Betsapi : BaseJob<UpdateOdds_VolleyballJob_Betsapi> {
	private const string JOB_NAME = nameof(UpdateOdds_VolleyballJob_Betsapi);

	internal static void Register(IServiceCollectionQuartzConfigurator quartzConfig, AppSetting appSetting) {
		quartzConfig.ScheduleJob<UpdateOdds_VolleyballJob_Betsapi>(trigger => trigger
	...
	public UpdateOdds_VolleyballJob_Betsapi(
		AppDbContext dbContext,
		IOptionsSnapshot<AppSetting> snapshot,
		ILogger<UpdateOdds_VolleyballJob_Betsapi> logger,
	...
	public override async Task Run(IJobExecutionContext context) {
		...
		var queryLiveMatches =
			...
			where _league.sport_id == MstSportModelConst.Id_Volleyball
	...
	private async Task _UpdateMatchOddAsync(SportMatchModel sysMatch) {
		var apiResult = await this.betsapiRepo.FetchMatchOddsSummary<Betsapi_VolleyballOddsSummaryData>(sysMatch.ref_betsapi_match_id);
		if (apiResult is null || apiResult.failed) {
			this.logger.ErrorDk(this, $"Fetch Volleyball match odds failed", $"sysMatchId: {sysMatch.id}, apiResult: {apiResult}");
	...
	private void _UpdateOdd_MainFullTime(
		...
		Betsapi_VolleyballOddsSummaryData apiResult
	) {
		...
		var odd_365 = bookmaker.Bet365?.odds?.end?._91_1 ?? bookmaker.Bet365?.odds?.kickoff?._91_1 ?? bookmaker.Bet365?.odds?.start?._91_1;
		if (odd_365 != null) {
		...
		var odd_1xbet = bookmaker._1XBet?.odds?.end?._91_1 ?? bookmaker._1XBet?.odds?.kickoff?._91_1 ?? bookmaker._1XBet?.odds?.start?._91_1;
		if (odd_1xbet != null) {
