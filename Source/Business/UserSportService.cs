	...
	public async Task<ApiResponse> PlaceBet(Guid userId, int sport_id, Sport_PlaceBetRequestBody reqBody) {
		...
		var queryMatches =
			...
			where ((int)_league.sport_id) == sport_id &&
				targetMatchIds.Contains(_match.id) &&
				SportMatchModelConst.BETTABLE_MATCH_STATUS_LIST.Contains(_match.status)
		...
		if (matchResultItems.Count != reqBody.bets.Length) {
			return new ApiBadRequestResponse("Invalid request data (contain invalid match?)") { code = ErrCode.invalid_data };
		}

		//todo impl: accept odd change
		...
		foreach (var req_bet in reqBody.bets) {
			...
			var name2market = match.markets.ToDictionary(m => m.name);
			...
			foreach (var req_market in req_bet.markets) {
				...
				foreach (var req_odd in req_market.odds) {
					...
					if (odd.value != req_odd.value) {
						return new ApiBadRequestResponse("Odd changed, please reload and try new bet again") { code = ErrCode.odd_changed };
					}
		...
		if (reward_receiver_address is null) {
			reward_receiver_address = userWallet.wallet_address;
		}
	...
	private async Task<ApiResponse> _TrySubmitToChainAsync(List<SportUserBetModel> userBets) {
		...
		try {
			...
			var queryBets =
				...
				select new {
					...
					home_team_name = _team1.name_en,
					away_team_name = _team2.name_en,
		...
		// Update tx-status at finally block
			...
			foreach (var ubet in userBets) {
				...
				ubet.submit_tx_result_message = txResultMessage.TruncateIfLongDk();
			}
			await this.dbContext.SaveChangesAsync();
		}
	}
