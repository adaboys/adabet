          ...
          <v-card-text>
            <v-form v-model="formIsValid" @submit.prevent="update">
              ...
              <form-parameter
                ...
                :form="form"
                form-key="fields"
                @change="clearFormErrors($event,`fields.field.id`)"
              />
...
import { objectPropertiesAreDifferent } from '~/plugins/utils'

export default {
  components: { FormParameter, ImageUpload },
  ...
  computed: {
    ...
    changed () {
      return objectPropertiesAreDifferent(this.form.keys(), this.user, ['name', 'email', 'hide_profit']) ||
        objectPropertiesAreDifferent(this.form.fields, this.user.fields)
    },
    fields () {
  ...
  created () {
    this.form.keys().forEach(key => {
      ...
      this.form[key] = value !== null && typeof value === 'object' ? { ...value } : value // note that, typeof null = 'object'
    })
  },
