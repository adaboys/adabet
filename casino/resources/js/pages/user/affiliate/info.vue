      ...
      <v-col cols="12" lg="6">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-text-field
              ref="link"
              :value="user.affiliate_url"
              :label="$t('Your affiliate link')"
              append-icon="mdi-content-copy"
              outlined
              readonly
              @click:append="copyToClipboard($refs.link.$el.querySelector('input'))"
            >
            ...
            <v-simple-table>
              <template #default>
                ...
                <tbody>
                  ...
                  <tr v-if="commission('raffle_ticket', 0) > 0 || commission('raffle_ticket', 1) > 0 || commission('raffle_ticket', 2) > 0">
                    <td class="text-left">{{ $t('Referral raffle ticket purchase') }}</td>
                    <td class="text-left">{{ $t('% of raffle ticket cost') }}</td>
                    <td class="text-right">{{ commission('raffle_ticket', 0) }}%</td>
                    <td class="text-right">{{ commission('raffle_ticket', 1) }}%</td>
                    <td class="text-right">{{ commission('raffle_ticket', 2) }}%</td>
                  </tr>
                </tbody>
              </template>
            </v-simple-table>
