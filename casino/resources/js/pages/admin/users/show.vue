        ...
        <v-card>
          ...
          <v-card-text>
            <key-value-table
              ...
              :headers="headers"
            >
              <template #referrer="{ item: user }">
                <router-link v-if="user.referrer" :to="{ name: 'admin.users.show', params: { id: user.referrer.id } }">
                  {{ user.referrer.name }}
                </router-link>
              </template>
              <template #fields="{ item: user }">
                <template v-if="user.fields.length">
                  <div v-for="(value, key) in user.fields" :key="key" class="d-flex justify-space-between">
                    <div class="my-2">
                      {{ key }}
                    </div>
                    <div class="my-2">
              ...
              <template #profiles="{ item: user }">
                <template v-if="user.profiles.length">
                  <div v-for="(profile, key) in user.profiles" :key="key">
                    <span class="text-uppercase">{{ profile.provider_name }}</span>
                    <span>{{ profile.provider_user_id }}</span>
                  </div>
                </template>
                <template v-else>
                  &nbsp;
              ...
              <template #notes="{ item: user }">
                <span style="white-space: pre;">{{ user.notes }}</span>
              </template>
            </key-value-table>
          </v-card-text>
        </v-card>
  ...
  computed: {
    headers () {
      return [
        ...
        { text: this.$t('Auth profiles'), value: 'profiles' },
        { text: this.$t('Referrer'), value: 'referrer' },
        { text: this.$t('Avatar'), value: 'avatar' },
        { text: this.$t('Name'), value: 'name' },
        { text: this.$t('Email'), value: 'email' },
        { text: this.$t('Extra fields'), value: 'fields' },
