          ...
          <v-tabs vertical>
              ...
              <v-card flat>
                <v-card-text>
                  ...
                  <v-text-field
                    ...
                    :label="$t('App URL')"
                    :rules="[validationRequired]"
                    :error="form.errors.has('APP_URL')"
                    :error-messages="form.errors.get('APP_URL')"
                    outlined
                    @keydown="clearFormErrors($event,'APP_URL')"
                  />
              ...
              <v-card flat>
                <v-card-text>
                  ...
                  <template v-for="bg in backgrounds">
                    <color-picker :key="bg" v-model="form[bg]" :label="backgroundText(bg)" />
                  </template>

                  <v-select
                    v-model="form.THEME_BACKGROUND"
                    :items="animatedBackgrounds"
                    :label="$t('Background animation')"
                  ...
                  <v-text-field
                    v-model="form.THEME_FONT_BODY_URL"
                    :label="$t('Body font URL')"
                    outlined
                  />

                  <v-text-field
                    v-model="form.THEME_FONT_BODY_FAMILY"
                    :label="$t('Body font family name')"
                    outlined
                  />

                  <v-text-field
                    v-model="form.THEME_FONT_HEADING_URL"
                    :label="$t('Heading font URL')"
                    outlined
                  />

                  <v-text-field
                    v-model="form.THEME_FONT_HEADING_FAMILY"
                    :label="$t('Heading font family name')"
                    outlined
                  />
                </v-card-text>
              </v-card>
                  ...
                  <v-expansion-panels>
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        ...
                        <v-switch
                          v-model="form.INTERFACE_SOUND_ENABLED_BY_DEFAULT"
                          :label="$t('Enable sound by default')"
                          color="primary"
                          false-value="false"
                          true-value="true"
                          class="mb-5"
                    ...
                    <v-expansion-panel>
                      <v-expansion-panel-header>{{ $t('Game feed') }}</v-expansion-panel-header>
                      <v-expansion-panel-content>
                        <v-switch
                          v-model="form.INTERFACE_GAME_FEED_ENABLED"
                          :label="$t('Enabled')"
                          color="primary"
                          false-value="false"
                          true-value="true"
                        ...
                        <v-switch
                          v-if="form.INTERFACE_GAME_FEED_ENABLED === 'true'"
                          v-model="form.INTERFACE_GAME_FEED_ENABLED_BY_DEFAULT"
                          :label="$t('Enable for users by default')"
                          color="primary"
                          false-value="false"
                          true-value="true"
                          :disabled="!pusherEnabled"
                          :hint="pusherEnabled ? $t('Users can also turn it on and off individually in the settings') : $t('Please fill Pusher settings first under Services tab.')"
                          :persistent-hint="true"
                          class="mb-5"
                        />
                      </v-expansion-panel-content>
                    ...
                    <v-expansion-panel>
                      <v-expansion-panel-header>{{ $t('Tips') }}</v-expansion-panel-header>
                      <v-expansion-panel-content>
                        <v-row>
                          <v-col>
                            <v-switch
                        ...
                        <v-row v-if="form.TIPS_ENABLED === 'true'">
                          <v-col cols="12" md="6">
                            <v-text-field
                              v-model.number="form.TIPS_MAX_AMOUNT"
                              :label="$t('Max amount')"
                              :rules="[validationPositiveInteger]"
                              :error="form.errors.has('TIPS_MAX_AMOUNT')"
                              :error-messages="form.errors.get('TIPS_MAX_AMOUNT')"
                              :prefix="$t('up to')"
                              :suffix="$t('credits')"
                              outlined
                              class="text-center"
                              @keydown="clearFormErrors($event,'TIPS_MAX_AMOUNT')"
                            />
                          </v-col>
                          <v-col cols="12" md="6">
                            <v-text-field
                              v-model.number="form.TIPS_INTERVAL"
                              :label="$t('Interval')"
                              :rules="[validationPositiveInteger]"
                              :error="form.errors.has('TIPS_INTERVAL')"
                              :error-messages="form.errors.get('TIPS_INTERVAL')"
                              :prefix="$t('every')"
                              :suffix="$t('hours')"
                              outlined
                              class="text-center"
                              @keydown="clearFormErrors($event,'TIPS_INTERVAL')"
                            />
                          </v-col>
                        </v-row>
                      </v-expansion-panel-content>
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        <v-expansion-panels>
                          ...
                          <v-expansion-panel>
                            ...
                            <v-expansion-panel-content>
                              <v-text-field
                                ...
                                :label="$t('Height on large screens')"
                                :rules="[validationInteger, validationPositiveNumber]"
                                :suffix="$t('px')"
                                outlined
                                class="mt-3"
                              ...
                              <v-text-field
                                v-model.number="form.CONTENT_HOME_SLIDER.height_mobile"
                                :label="$t('Height on small screens')"
                                :rules="[validationInteger, validationPositiveNumber]"
                                :suffix="$t('px')"
                                outlined
                              />
                              ...
                              <v-switch
                                v-model="form.CONTENT_HOME_SLIDER.overlay"
                                :label="$t('Dark overlay')"
                                color="primary"
                                :false-value="false"
                                :true-value="true"
                              />
                          ...
                          <v-expansion-panel v-if="gameProvidersPackageIsEnabled">
                            ...
                            <v-expansion-panel-content>
                              <v-expansion-panels>
                                <v-expansion-panel>
                                  ...
                                  <v-expansion-panel-content>
                                    <v-card
                                      v-for="(category, k) in form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES"
                                      :key="k"
                                      outlined
                                      class="mb-5"
                                    >
                                      ...
                                      <v-card-text>
                                        <v-row>
                                          ...
                                          <v-col cols="12" md="6">
                                            <v-text-field
                                              v-model="form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES[k].icon"
                                              :label="$t('Icon')"
                                              :prepend-inner-icon="form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES[k].icon"
                                              :rules="[validationRequired]"
                                              :hint="$t('Choose icons at https://materialdesignicons.com')"
                                              :persistent-hint="true"
                                              outlined
                                          ...
                                          <v-col cols="12">
                                            <v-btn :disabled="k === featuredCategoriesCount - 1" color="primary" icon tile @click="moveFeaturedCategoryDown(k)">
                                              <v-icon>mdi-arrow-down</v-icon>
                                            </v-btn>
                                            <v-btn :disabled="k === 0" color="primary" icon tile @click="moveFeaturedCategoryUp(k)">
                                              <v-icon>mdi-arrow-up</v-icon>
                                            </v-btn>
                                            <v-btn :disabled="featuredCategoriesCount <= 1" icon tile color="error">
                                              <v-icon @click="deleteFeaturedCategory(k)">
                                                mdi-delete
                                              </v-icon>
                                            </v-btn>
                                          </v-col>
                                ...
                                <v-expansion-panel>
                                  <v-expansion-panel-header>{{ $t('Display') }}</v-expansion-panel-header>
                                  <v-expansion-panel-content>
                                    <v-select
                          ...
                          <v-expansion-panel v-if="rafflePackageIsEnabled">
                            <v-expansion-panel-header>{{ $t('Raffles list') }}</v-expansion-panel-header>
                            <v-expansion-panel-content>
                              <v-select
                                v-model="form.CONTENT_HOME_RAFFLES_DISPLAY_STYLE"
                                :items="raffleDisplayStyles"
                                :label="$t('Display style')"
                                outlined
                              />
                            </v-expansion-panel-content>
                  ...
                  <v-expansion-panels>
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        ...
                        <v-switch
                          v-model="form.USERS_CREATE_RANDOM_AVATAR"
                          :label="$t('Auto create an avatar for new users')"
                          color="primary"
                          false-value="false"
                          true-value="true"
                        />
                        ...
                        <v-expansion-panels>
                          <v-expansion-panel>
                            <v-expansion-panel-header>
                              {{ $t('Sign-up form fields') }}
                              <span v-if="form.USERS_FIELDS.length" class="ml-2">
                                ({{ form.USERS_FIELDS.length }})
                              </span>
                            </v-expansion-panel-header>
                            <v-expansion-panel-content>
                              <entity-parameters v-model="form.USERS_FIELDS">
                                <template #description>
                                  <p>
                                    {{ $t('Add any number of extra fields that should be present on the sign up form and filled in by the user.') }}
                                  </p>
                                </template>
                              </entity-parameters>
                            </v-expansion-panel-content>
              ...
              <v-card flat>
                <v-card-text>
                  <v-expansion-panels>
                    ...
                    <v-expansion-panel v-if="paymentsPackageIsEnabled">
                      <v-expansion-panel-header>{{ $t('Deposit') }}</v-expansion-panel-header>
                      <v-expansion-panel-content>
                          <p class="mb-3">
                            {{ $t('First deposit') }}
                          </p>
                          <v-row>
                            <v-col cols="12" md="2">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_FIRST_PCT"
                                :label="$t('Amount')"
                                :suffix="$t('%')"
                                :rules="[validationNonNegativeNumber]"
                                :error="form.errors.has('BONUSES_DEPOSIT_FIRST_PCT')"
                                :error-messages="form.errors.get('BONUSES_DEPOSIT_FIRST_PCT')"
                                outlined
                              />
                            </v-col>
                            <v-col cols="12" md="6">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_FIRST_THRESHOLD"
                                :label="$t('Condition')"
                                :prefix="$t('Deposit') + ' >= '"
                                :suffix="$t('credits')"
                                :rules="[validationPositiveInteger]"
                                :error="form.errors.has('BONUSES_DEPOSIT_FIRST_THRESHOLD')"
                                :error-messages="form.errors.get('BONUSES_DEPOSIT_FIRST_THRESHOLD')"
                                class="text-center"
                                outlined
                              />
                            </v-col>
                            <v-col cols="12" md="4">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_FIRST_MAX"
                                :label="$t('Max amount')"
                                :suffix="$t('credits')"
                                :rules="[validationPositiveInteger]"
                                :error="form.errors.has('BONUSES_DEPOSIT_FIRST_MAX')"
                                :error-messages="form.errors.get('BONUSES_DEPOSIT_FIRST_MAX')"
                                outlined
                              />
                            </v-col>
                          </v-row>
                        <p class="mb-5">
                          {{ $t('Consecutive deposits') }}
                        </p>
                        <v-row>
                          <v-col cols="12" md="2">
                            <v-text-field
                          ...
                          <v-col cols="12" md="4">
                            <v-text-field
                              v-model.number="form.BONUSES_DEPOSIT_MAX"
                              :label="$t('Max amount')"
                              :suffix="$t('credits')"
                              :rules="[validationPositiveInteger]"
                              :error="form.errors.has('BONUSES_DEPOSIT_MAX')"
                              :error-messages="form.errors.get('BONUSES_DEPOSIT_MAX')"
                              outlined
                            />
                          </v-col>
                        </v-row>
                        <div v-if="gameProvidersPackageIsEnabled">
                          <p class="mb-5">
                            {{ $t('Bonus rounds') }}
                          </p>
                          <v-row>
                            <v-col cols="12" md="6">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_BONUS_ROUNDS_THRESHOLD"
                                :label="$t('Condition')"
                                :prefix="$t('Deposit') + ' >= '"
                                :suffix="$t('credits')"
                                :rules="[validationPositiveInteger]"
                          ...
                          <v-row>
                            <v-col cols="12" md="4">
                              <v-select
                                v-model="form.BONUSES_DEPOSIT_BONUS_ROUNDS_PROVIDER"
                                :items="[{ value: 'evoplay', text: 'Evoplay' }]"
                                :label="$t('Provider')"
                                :disabled="true"
                                outlined
                              />
                            ...
                            <v-col cols="12" md="3">
                              <v-text-field
                                v-model="form.BONUSES_DEPOSIT_BONUS_ROUNDS_GAME_ID"
                                :label="$t('Game ID')"
                                outlined
                              />
                            </v-col>
                            <v-col cols="12" md="3">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_BONUS_ROUNDS_COUNT"
                                :label="$t('Rounds')"
                                outlined
                              />
                            </v-col>
                            <v-col cols="12" md="2">
                              <v-text-field
                                v-model.number="form.BONUSES_DEPOSIT_BONUS_ROUNDS_BET"
                                :label="$t('Bet')"
                                outlined
                              />
                            </v-col>
                  ...
                  <v-expansion-panels>
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        ...
                        <v-switch
                          v-model="form.AFFILIATE_HASH_USER_ID"
                          :label="$t('Hash user ID in affiliate links')"
                          color="primary"
                          false-value="false"
                          true-value="true"
                        />
                      ...
                      <v-expansion-panel-content>
                        <template v-if="form.AFFILIATE_COMMISSIONS_SIGN_UP">
                          <div v-for="tier in [0,1,2]" :key="'tier-' + tier">
                            ...
                            <v-text-field
                              v-model.number="form.AFFILIATE_COMMISSIONS_RAFFLE_TICKET[tier]"
                              :label="$t('Raffle ticket bonus')"
                              :rules="[validationNonNegativeNumber]"
                              :error="form.errors.has('AFFILIATE_COMMISSIONS_RAFFLE_TICKET')"
                              :error-messages="form.errors.get('AFFILIATE_COMMISSIONS_RAFFLE_TICKET')"
                              :suffix="$t('% of raffle ticket cost')"
                              outlined
                            />
                  ...
                  <v-expansion-panels>
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        ...
                        <v-text-field
                          v-if="form.API_CRYPTO_PROVIDER === 'coincap'"
                          v-model="form.API_CRYPTO_PROVIDERS_COINCAP_API_KEY"
                          :label="$t('API key')"
                          :error="form.errors.has('API_CRYPTO_PROVIDERS_COINCAP_API_KEY')"
                          :error-messages="form.errors.get('API_CRYPTO_PROVIDERS_COINCAP_API_KEY')"
                          outlined
                        >
                          <template v-slot:append>
                            <v-btn small color="primary" href="https://coincap.io/api-key" target="_blank">
                              {{ $t('Get a free key') }}
                            </v-btn>
                          </template>
                        </v-text-field>
                        <v-text-field
                          v-else-if="form.API_CRYPTO_PROVIDER === 'cryptocompare'"
              ...
              <v-card flat>
                <v-card-text>
                  <v-expansion-panels>
                    ...
                    <v-expansion-panel v-for="provider in web3AuthProviders" :key="provider">
                      <v-expansion-panel-header>{{ ucfirst(provider) }}</v-expansion-panel-header>
                      <v-expansion-panel-content>
                        <v-switch
                          v-model="form[web3AuthProviderEnabledVariable(provider)]"
                          :label="$t('Enabled')"
                          color="primary"
                          false-value="false"
                          true-value="true"
            ...
            <v-tab-item>
              <v-card flat>
                <v-card-text>
                  <v-expansion-panels>
                    <v-expansion-panel>
                      <v-expansion-panel-header>{{ $t('SMTP') }}</v-expansion-panel-header>
                      <v-expansion-panel-content>
                        <v-text-field
            ...
            <template v-for="(pkg, id) in packages">
              <v-tab-item v-if="packageComponents[id]" :key="id">
                <component :is="packageComponents[id]" :package-id="id" :form="form" @input="mergePackageConfig" />
              </v-tab-item>
            </template>
          </v-tabs>
...
import { get, ucfirst, copyToClipboard, moveArrayElement, baseUrl } from '~/plugins/utils'
...
export default {
  ...
  components: { EntityParameters, Stars, FileEditor, FileUpload, ColorPicker },
  ...
  data () {
    ...
    return {
      ...
      form: new Form({
        ...
        APP_URL: config('app.url') || baseUrl(),
        ...
        THEME_BACKGROUND_APP_BAR: config('settings.theme.backgrounds.app_bar'), // $t('App bar background')
        THEME_BACKGROUND_SYSTEM_BAR: config('settings.theme.backgrounds.system_bar'), // $t('System bar background')
        THEME_BACKGROUND_PAGE: config('settings.theme.backgrounds.page'), // $t('Page background')
        THEME_BACKGROUND_NAV_BAR: config('settings.theme.backgrounds.nav_bar'), // $t('Nav bar background')
        THEME_BACKGROUND_FOOTER: config('settings.theme.backgrounds.footer'), // $t('Footer background')
        THEME_BACKGROUND: config('settings.theme.background'),
        THEME_FONT_BODY_URL: config('settings.theme.fonts.body.url'),
        THEME_FONT_BODY_FAMILY: config('settings.theme.fonts.body.family'),
        THEME_FONT_HEADING_URL: config('settings.theme.fonts.heading.url'),
        THEME_FONT_HEADING_FAMILY: config('settings.theme.fonts.heading.family'),
        ...
        CONTENT_HOME_RAFFLES_DISPLAY_STYLE: config('settings.content.home.raffles.display_style'),
        ...
        INTERFACE_SOUND_ENABLED_BY_DEFAULT: '' + config('settings.interface.sound.enabled_by_default'),
        INTERFACE_GAME_FEED_ENABLED: '' + config('settings.interface.game_feed.enabled'),
        INTERFACE_GAME_FEED_ENABLED_BY_DEFAULT: '' + config('settings.interface.game_feed.enabled_by_default'),
        ...
        CHAT_MESSAGE_MAX_LENGTH: config('settings.interface.chat.message_max_length'),
        TIPS_ENABLED: '' + config('settings.tips.enabled'),
        TIPS_MAX_AMOUNT: config('settings.tips.max_amount'),
        TIPS_INTERVAL: config('settings.tips.interval'),
        ...
        BONUSES_DEPOSIT_FIRST_PCT: config('settings.bonuses.deposit.first.pct'),
        BONUSES_DEPOSIT_FIRST_THRESHOLD: config('settings.bonuses.deposit.first.threshold'),
        BONUSES_DEPOSIT_FIRST_MAX: config('settings.bonuses.deposit.first.max'),
        BONUSES_DEPOSIT_PCT: config('settings.bonuses.deposit.regular.pct'),
        BONUSES_DEPOSIT_THRESHOLD: config('settings.bonuses.deposit.regular.threshold'),
        BONUSES_DEPOSIT_MAX: config('settings.bonuses.deposit.regular.max'),
        BONUSES_DEPOSIT_BONUS_ROUNDS_THRESHOLD: config('settings.bonuses.deposit.bonus_rounds.threshold'),
        BONUSES_DEPOSIT_BONUS_ROUNDS_PROVIDER: config('settings.bonuses.deposit.bonus_rounds.provider'),
        BONUSES_DEPOSIT_BONUS_ROUNDS_GAME_ID: config('settings.bonuses.deposit.bonus_rounds.game_id'),
        BONUSES_DEPOSIT_BONUS_ROUNDS_COUNT: config('settings.bonuses.deposit.bonus_rounds.count'),
        BONUSES_DEPOSIT_BONUS_ROUNDS_BET: config('settings.bonuses.deposit.bonus_rounds.bet'),
        AFFILIATE_AUTO_APPROVAL_FREQUENCY: config('settings.affiliate.auto_approval_frequency'),
        AFFILIATE_HASH_USER_ID: '' + config('settings.affiliate.hash_user_id'),
        ...
        AFFILIATE_COMMISSIONS_RAFFLE_TICKET: config('settings.affiliate.commissions.raffle_ticket.rates'),
        ...
        EMAIL_VERIFICATION: '' + config('settings.users.email_verification'),
        USERS_CREATE_RANDOM_AVATAR: '' + config('settings.users.create_random_avatar'),
        USERS_FIELDS: config('settings.users.fields'),
        ...
        API_CRYPTO_PROVIDERS_COINCAP_API_KEY: config('services.api.crypto.providers.coincap.api_key'),
        ...
        ...Object.keys(config('auth.web3')).reduce((o, provider) => {
          o[`AUTH_WEB3_${provider.toUpperCase()}_ENABLED`] = '' + config(`auth.web3.${provider}.enabled`)
          return o
        }, {})
      }),
      packageComponents: {},
  ...
  computed: {
    ...
    backgrounds () {
      return Object.keys(this.form).filter(v => v.startsWith('THEME_BACKGROUND_'))
    },
    colors () {
    ...
    raffleDisplayStyles () {
      return [
        { value: 'card', text: this.$t('Card') },
        { value: 'card2', text: this.$t('Card 2') },
        { value: 'card3', text: this.$t('Card 3') }
      ]
    },
    chatRainFrequencies () {
      return [
    ...
    web3AuthProviders () {
      return Object.keys(config('auth.web3'))
    },
    pusherEnabled () {
    ...
    featuredCategoriesCount () {
      return get(this.form, 'CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES', []).length
    }
  },
  ...
  methods: {
    ...
    addFeaturedCategory () {
      this.form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES.push({ title: 'Title', icon: 'mdi-cards-spade', categories: [] })
    },
    moveFeaturedCategoryUp (i) {
      moveArrayElement(this.form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES, i, i - 1)
    },
    moveFeaturedCategoryDown (i) {
      moveArrayElement(this.form.CONTENT_HOME_PROVIDER_GAMES_FEATURED_CATEGORIES, i, i + 1)
    },
    ...
    web3AuthProviderEnabledVariable (provider) {
      return `AUTH_WEB3_${provider.toUpperCase()}_ENABLED`
    },
    providerRedirectUrlValue (provider) {
      return config('app.url') + config(`oauth.${provider}.redirect`)
    },
    backgroundText (v) {
      const s = v.replace('THEME_BACKGROUND_', '').replace('_', ' ').toLowerCase() + ' background'
