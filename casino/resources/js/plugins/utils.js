...
export function countDecimals (value) {
  if (value === null || Math.floor(value.valueOf()) === value.valueOf()) return 0

  const str = value.toString()

  if (str.indexOf('e-') > -1) {
    const nums = str.split('e-')
    return parseInt(nums[1], 10) + (parseFloat(nums[0]) > 1 ? 1 : 0)
 ...
 * @returns string
 */
export function hex2ascii (str) {
  return str.match(/.{1,2}/g).map(v => String.fromCharCode(parseInt(v, 16))).join('')
}

/**
 * Check if properties of 2 objects are different
 *
 * @param obj
 * @param obj2
 * @param props
 * @returns {*}
 */
export function objectPropertiesAreDifferent (obj, obj2, props = null) {
  const keys = props || Object.keys(obj)
  return keys.some(key => obj[key] !== obj2[key])
}
