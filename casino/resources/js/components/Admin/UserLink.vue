  ...
  <span class="text-no-wrap">
    ...
    <v-tooltip v-for="profile in user.profiles" :key="profile.id" bottom>
      <template #activator="{ on }">
        <v-icon small v-on="on">mdi-{{ providers[profile.provider_name] ? (providers[profile.provider_name].mdi || profile.provider_name) : 'wallet' }}</v-icon>
      </template>
      <span>{{ profile.provider_name }} {{ $t('ID') }} {{ profile.provider_user_id }}</span>
    </v-tooltip>
