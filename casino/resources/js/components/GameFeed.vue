<template>
  <div>
    <template v-for="(game, i) in Object.keys(games).map(k => games[k])">
      <v-hover v-slot="{ hover }" :key="game.id">
        <v-snackbar
          app
          left
...
export default {
  ...
  created () {
    if (this.enabledBySystem && this.enabledByUser && !this.isMobile) {
      this.subscribe()
    }
  },
  ...
  methods: {
    ...
    subscribe () {
      this.unsubscribe = this.$store.subscribe(({ type, payload: game }, state) => {
        if (type === `game/${GAME_ADD}` && this.user.id !== game.account.user.id) {
