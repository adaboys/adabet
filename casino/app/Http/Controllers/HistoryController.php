...
class HistoryController extends Controller
{
    ...
    public function recent(GameHistoryQuery $query)
    {
        $items = $query
            ...
            ->with('account:id,user_id', 'account.user:id,name,email,avatar,last_seen_at') 
            ->get()
    ...
    public function show(GetGame $request, Game $game)
    {
        ...
        if (method_exists($game->gameable, 'getVisibleOnlyToAdmins')) {
            $game->gameable->makeHidden(!$request->user()->is_admin ? $game->gameable->getVisibleOnlyToAdmins() : []);
        }
